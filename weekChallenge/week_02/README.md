...
좋은 코드란 무엇일까? 코딩에 정답은 없는 것인지 고민을 많이 했다. SQLAlchemy 방식과 SQLModel 방식 중에 어떤걸 선택할지, 클래스 + @staticethod 방식과 순수 함수 방식 중에 어떤걸 선택할지 리턴값으로 생성하는 메시지는 왜 라우터에 있는지 등등.. 내가 생각한 코드와 다른 학생들의 코드, AI가 생각한 코드들이 다른 경우가 많았고 어떤 것이 정답인지 헷갈렸다. 

기술과 아키텍처의 선택은 언제나 트레이드오프가 존재하고 이를 잘 고려해서 코딩을 해야한다고 알고 있지만 아직 감각이 없는 것 같다. 일단 나만의 기준을 세우고 하나의 방법으로 쭉 코딩해본 다음 부족한 부분이나 수정할만한 부분을 잘 찾아서 바꿔나가보려고 한다. 
