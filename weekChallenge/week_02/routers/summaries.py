...
import controllers.summary as summary_controller
...
def ai_summaries(post_id: int, db: Session = Depends(get_db)):
    ai_summary_text = summary_controller.create_or_update_ai_summary(db, post_id)
    return {"message": f"{post_id}에 AI 요약이 성공적으로 만들어졌습니다.", "data": ai_summary_text}
